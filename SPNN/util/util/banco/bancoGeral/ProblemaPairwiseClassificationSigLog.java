...
public class ProblemaPairwiseClassificationSigLog extends TipoProblema{


	private int totalRodadas;
	
	public ProblemaPairwiseClassificationSigLog(int qtClasses) {
		super(qtClasses);
	...
	private int votacao(List<Integer> classes, double[][] outputs){		
		...
		while(classesCopy.size() > 1 && tamanhoAnt != classesCopy.size()){
			...
			System.out.println("Mapa Pontos");
			for (Integer integer : mapa.keySet()) {
				System.out.println(integer + ", pt: "+mapa.get(integer));
			}
	...
	public String toString() {
		return "Pairwise Classification SigmLog";
	}
	...
	public static void main(String[] args) {
		...
		int qtClasses = 5;
		ProblemaPairwiseClassificationSigLog problema = new ProblemaPairwiseClassificationSigLog(qtClasses);
		...
		int classe = problema.classifica(new double[][]{{0.1},
											{0.7},
											{0.8},
											{0.1},
											{0.2},
